...
v2 is phased. See `ROADMAP.md` for full design rationale. Monetization deferred to v3.
...
- On-demand question sets: unlimited batches generated from cached transcript on request; stored with `questions.setIndex` (0 = original, 1+ = on-demand); pro-only gate deferred to v3 monetization
- Question flagging: single-tap flag writes to new `question_flags` table; creators see flag counts per question on edit page
- Completion records: new `course_completions` table; shareable `/learn/[courseId]/complete` page; dashboard shows completed courses separately

**Phase 3 — Discovery**
- Public catalog at `/catalog` — anonymous browsing, course cards, `unstable_cache` with short TTL
- Keyword search via Postgres `ILIKE` on `courses.title` and `courses.topic`
- Filter chips for Quick / Standard / Long (subject-area filter deferred — no usage data yet)
- `generateMetadata` on public course pages for SEO + social sharing; private pages get `noindex`

**Dropped from earlier backlog**
- ~~Creator: review and edit curriculum before video search runs~~ — removed; violates AI-first product philosophy (if you know the right curriculum, you'd build a playlist manually)
