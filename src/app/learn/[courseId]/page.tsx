            ...
            >
              <div className="w-7 h-7 rounded-full flex items-center justify-center shrink-0 mt-0.5 text-[13px] font-semibold">
                {state === "complete" ? (
                  ...
                  <span className={state === "current" ? "text-[#F05A2A]" : "text-[--text-secondary]"}>{i + 1}</span>
                )}
              </div>
              ...
              {(state === "current" || state === "complete") && lesson.youtubeVideoId && (
                <Link
                  ...
                  className={`shrink-0 inline-flex items-center justify-center min-h-[36px] px-4 py-2 rounded-lg text-[14px] font-medium transition-colors ${
                    state === "current"
                      ? "bg-gradient-to-r from-[#F03C2E] to-[#FF7C1F] text-white hover:opacity-90"
                      : "border border-green-500/40 text-green-700 hover:bg-green-50"
                  }`}
                >
